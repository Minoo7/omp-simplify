...
export function createAutoSimplifyHooks(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", (event: BeforeAgentStartEventLike) => {
  ...
  pi.on("tool_execution_end", (event: ToolExecutionEndEventLike) => {
    const status = getWriteToolStatus(event.toolName);
    if (event.isError || !status) return;

    const path = extractEditedPath(event);
